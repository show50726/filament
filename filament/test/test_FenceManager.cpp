...
TEST_F(FenceManagerTest, TrackNonEmptySet) {
    ...
    //EXPECT_CALL(mMockDriver, createFence(_, _)).Times(1);
    //executeCommands();
}
...
TEST_F(FenceManagerTest, ReclaimWhenFenceSignaled) {
    ...
    EXPECT_NE(std::find(reclaimedIds.begin(), reclaimedIds.end(), 10), reclaimedIds.end());
    EXPECT_NE(std::find(reclaimedIds.begin(), reclaimedIds.end(), 20), reclaimedIds.end());
