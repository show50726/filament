...
void groundTruthAmbientOcclusion(out float obscurance, out vec3 bentNormal,
    ...
    for (int i = 0; i <= materialParams.sliceCount; i++) {
        ...
        omega *= ssRadius;
        const vec3 directionV = vec3(cosPhi, sinPhi);
        const vec3 orthoDirectionV = directionV - (dot(directionV, viewDir)*viewDir);
        const vec3 axisV = normalize(cross(directionV, viewDir));
        const vec3 projNormalV = normal - axisV * dot(projNormalV, axisV);

        const float signNorm = (float)sign(dot(orthoDirectionV, projNormalV));
        const float projNormalLength = length(projNormalV);
        const float cosNorm = (float)saturate(dot(projNormalV, viewDir) / projNormalLength);
        ...
        float horizonCos0 = -1;
        float horizonCos1 = -1;
        for (int j = 0; j < materialParams.stepsPerSlice; j++) {
            float step = j / materialParams.stepsPerSlice;
            vec2 sampleOffset = step * omega;

            // TODO: sample Hi-Z
            float2 sampleScreenPos0 = uv + sampleOffset;
            highp float sampleDepth0 = sampleDepthLinear(materialParams_depth, sampleScreenPos0, 0);
            ...
            highp float sampleDepth1 = sampleDepthLinear(materialParams_depth, sampleScreenPos1, 0);
            highp vec3 samplePos1 = computeViewSpacePositionFromDepth(sampleScreenPos1, sampleDepth1,
                materialParams.positionParams);

            float sampleHorizonV0 = normalize(samplePos0 - origin);
            float sampleHorizonV1 = normalize(samplePos1 - origin);
    ...
    occlusion = saturate(pow(occlusion / (float)materialParams.sliceCount), materialParams.power);
