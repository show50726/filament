...
public:
    ...
    backend::Handle<backend::HwProgram> prepareProgram(backend::DriverApi& driver,
            Variant const variant, uint32_t subKey,
            backend::CompilerPriorityQueue const priorityQueue) const noexcept {
        flushSpecializationConstants();
        return getPrograms().prepareProgram(driver, variant, subKey, priorityQueue);
    }
    ...
    [[nodiscard]]
    backend::Handle<backend::HwProgram> getProgram(Variant const variant, uint32_t subKey) const noexcept {
#if FILAMENT_ENABLE_MATDBG
        updateActiveProgramsForMatdbg(variant);
#endif
        return getPrograms().getProgram(variant, subKey);
    }
